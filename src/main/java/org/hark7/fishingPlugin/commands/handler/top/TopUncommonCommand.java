...
/**
 * コマンドハンドラー: top uncommon
 * <p>
 * プレイヤーの「UNCOMMON」レアリティの魚の数をランキング形式で表示するコマンドを処理します。
 */
public class TopUncommonCommand implements ICommandHandler {
     ...
     */
    public TopUncommonCommand(PlayerDataController manager) {
    ...
    private void sendLank(Player sender, List<Map.Entry<UUID, Integer>> sortedList) {
        ...
        var title = CustomLang.of("Commands.top.uncommon.Title", lang);
        ...
        for (int i = 0; i < Math.min(10, sortedList.size()); i++) {
            ...
            CustomLang.ofSimpleComponent("Commands.top.uncommon.Content", lang)
