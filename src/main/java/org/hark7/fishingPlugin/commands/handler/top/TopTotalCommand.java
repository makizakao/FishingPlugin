 ...
 * コマンドハンドラー: top total
 * <p>
 * プレイヤーの全レアリティの魚の数をランキング形式で表示するコマンドを処理します。
 */
public class TopTotalCommand implements ICommandHandler {
     ...
     */
    public TopTotalCommand(PlayerDataController manager) {
    ...
    public void execute(Player player, String[] args) {
        ...
        for (Map.Entry<UUID, PlayerData> entry : playerDataList.entrySet()) {
            ...
            sortedList.add(Map.entry(playerId, data.countAll()));
        }
    ...
    private void sendLank(Player sender, List<Map.Entry<UUID, Integer>> sortedList) {
        ...
        var title = CustomLang.of("Commands.top.total.Title", lang);
        ...
        for (int i = 0; i < Math.min(10, sortedList.size()); i++) {
            ...
            CustomLang.ofSimpleComponent("Commands.top.total.Content", lang)
