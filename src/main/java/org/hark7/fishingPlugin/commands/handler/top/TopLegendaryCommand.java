...
/**
 * コマンドハンドラー: top legendary
 * <p>
 * プレイヤーの「LEGENDARY」レアリティの魚の数をランキング形式で表示するコマンドを処理します。
 */
public class TopLegendaryCommand implements ICommandHandler {
     ...
     */
    public TopLegendaryCommand(PlayerDataController manager) {
    ...
    private void sendLank(Player sender, List<Map.Entry<UUID, Integer>> sortedList) {
        ...
        var title = CustomLang.of("Commands.top.legendary.Title", lang);
        ...
        for (int i = 0; i < Math.min(10, sortedList.size()); i++) {
            ...
            CustomLang.ofSimpleComponent("Commands.top.legendary.Content", lang)
