 ...
 * コマンドハンドラー: top rare
 * <p>
 * プレイヤーの「RARE」レアリティの魚の数をランキング形式で表示するコマンドを処理します。
 */
public class TopRareCommand implements ICommandHandler {
     ...
     */
    public TopRareCommand(PlayerDataController manager) {
    ...
    private void sendLank(Player sender, List<Map.Entry<UUID, Integer>> sortedList) {
        ...
        var title = CustomLang.of("Commands.top.rare.Title", lang);
        ...
        for (int i = 0; i < Math.min(10, sortedList.size()); i++) {
            ...
            CustomLang.ofSimpleComponent("Commands.top.rare.Content", lang)
