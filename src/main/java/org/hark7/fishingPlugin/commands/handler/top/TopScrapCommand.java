 ...
 * コマンドハンドラー: top scrap
 * <p>
 * プレイヤーの「SCRAP」レアリティの魚の数をランキング形式で表示するコマンドを処理します。
 */
public class TopScrapCommand implements ICommandHandler {
     ...
     */
    public TopScrapCommand(PlayerDataController manager) {
    ...
    private void sendLank(Player sender, List<Map.Entry<UUID, Integer>> sortedList) {
        ...
        var title = CustomLang.of("Commands.top.scrap.Title", lang);
        ...
        for (int i = 0; i < Math.min(10, sortedList.size()); i++) {
            ...
            CustomLang.ofSimpleComponent("Commands.top.scrap.Content", lang)
